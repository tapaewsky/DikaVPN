...
- (id)init {
    self = [super init];
    NSString *appGroup = @"group.Tapg1er.DikaVPN";
    id<DDLogFileManager> logFileManager = [[DDLogFileManagerDefault alloc] init];
    NSLog(@"%@", logFileManager.logsDirectory.description);
    ...
    _packetQueue = dispatch_queue_create("group.Tapg1er.DikaVPN", DISPATCH_QUEUE_SERIAL);
...
- (void)startTunnelWithOptions:(NSDictionary *)options
    ...
    if (options == nil) {
        NSError *error = [NSError errorWithDomain:NEVPNErrorDomain code:NEVPNErrorConfigurationUnknown userInfo:nil];
